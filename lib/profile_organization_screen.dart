...
class _ProfileOrganizationScreenState extends State<ProfileOrganizationScreen> {
  ...
  Future<void> _saveOrgData() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('users').doc(_uid).set(update, SetOptions(merge: true));
  ...
  void _toggleEdit() => setState(() => isEditMode = true);
        ...
        child: Column(
          children: [
            ...
            ListTile(
              ...
              onTap: () {
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Favourites tapped")));
              },
            ),
            const Spacer(),
            ListTile(
          ...
          child: Column(
            children: [
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    buildEditableField(nameController, isEditMode,
                        fontSize: 20,
                        color: Colors.white,
                        isCenter: true,
                    ...
                    buildEditableField(websiteController, isEditMode,
                        fontSize: 16,
                    ...
                    Padding(
                      ...
                      child: isEditMode
                          ? buildEditableField(descriptionController, true,
                          placeholder: "Description", color: Colors.white, maxLines: 3)
                          : Text(
              ...
              Container(
                ...
                child: Column(
                  children: [
                    buildInfoCard(icon: Icons.location_on, controller: addressController, editable: isEditMode, placeholder: "Address"),
                    const SizedBox(height: 12),
                    buildInfoCard(icon: Icons.email, controller: emailController, editable: isEditMode, placeholder: "Email"),
                    const SizedBox(height: 12),
                    buildInfoCard(icon: Icons.phone, controller: contactController, editable: isEditMode, placeholder: "Contact Number"),
                    const SizedBox(height: 12),
                    buildInfoCard(icon: Icons.calendar_today, controller: foundedController, editable: isEditMode, placeholder: "Founded Year"),
                    const SizedBox(height: 12),
                    buildInfoCard(icon: Icons.apartment, controller: orgTypeController, editable: isEditMode, placeholder: "Organization Type"),
  ...
  Widget buildInfoCard({required IconData icon, required TextEditingController controller, required bool editable, String? placeholder}) {
    return Container(
  ...
  Widget buildEditableField(TextEditingController controller, bool editable,
      {String? placeholder, double fontSize = 14, Color color = Colors.black, bool isCenter = false, int maxLines = 1, bool hideUnderline = false}) {
    return editable
        ? TextFormField(
      controller: controller,
      ...
      decoration: InputDecoration(
        ...
        enabledBorder: UnderlineInputBorder(
          borderSide: BorderSide(color: hideUnderline ? Colors.transparent : color.withOpacity(0.5), width: 1),
        ),
        focusedBorder: UnderlineInputBorder(
          borderSide: BorderSide(color: hideUnderline ? Colors.transparent : color, width: 1.2),
        ),
      ),
    )
        : Text(controller.text, textAlign: isCenter ? TextAlign.center : TextAlign.start, style: TextStyle(fontSize: fontSize, color: color));
  }
}
